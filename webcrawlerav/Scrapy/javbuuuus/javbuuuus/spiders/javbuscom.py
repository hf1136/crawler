...
class JavbuscomSpider(scrapy.Spider):
    name = 'javbuscom'
    ...
    def parse(self, response):
        ...
        print(f'第{self.pagenumber}页')
        deatilpages = response.xpath("//a[@class='movie-box']/@href").extract()
        #soup.select('a[id="next"]')
        nextpage = response.xpath("//a[@id='next']/@href").extract()
        print(nextpage)
        next_page_url = ''
        if nextpage:
            next_page_link = nextpage[0].split('/')[-2:]
        ...
        if next_page_url != "" and self.pagenumber < self.maxpage:
            self.pagenumber += 1
            #print(next_page_url)
